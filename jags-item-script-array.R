...
# 7: n_burnin (e.g., 500)
# 8: n_iter (e.g., 2000)
# 9: sample_index (e.g., 1, 2, 3... from SLURM)
...
sample_index     <- if (length(args) >= 9) as.integer(args[9]) else 1 # <-- NEW 9th ARG
...
# -------------------------------------------------------------------
# --- RUN THE SINGLE SAMPLE FOR THIS ARRAY JOB ---
# This replaces the old loop and aggregation logic
# -------------------------------------------------------------------

print(paste("--- Starting process_sample for index", sample_index, "---"))
result <- process_sample(sample_index)

# --- Save the individual R objects for this sample ---
# We will combine these later in a separate script
sample_save_path <- file.path(folder_name, sprintf("sample_result_%d.rds", sample_index))
saveRDS(result$sample, file = sample_save_path)

mice_save_path <- file.path(folder_name, sprintf("mice_result_%d.rds", sample_index))
saveRDS(result$mice_result, file = mice_save_path)

jags_save_path <- file.path(folder_name, sprintf("jags_result_%d.rds", sample_index))
saveRDS(result$jags_result, file = jags_save_path)

print(paste("--- Finished process_sample for index", sample_index, "---"))
print(paste("Individual sample results saved to .rds files."))
